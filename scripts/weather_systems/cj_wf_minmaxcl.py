...
if __name__ == '__main__':
    ...
    ds_ghi = xr.concat(ds_list, dim='time')
    ...
    wf_files = [f for f in wf_path.rglob(f'*{year}*.nc')]
    ...
    if dataset == 'himawari':
        ...
        ds_wf = ds_wf.sel(time=slice(f'{year}-01-01T00', f'{year}-12-31T10:30'))
    ...
    # add metadata
    masked_ghi_mean = masked_ghi_mean.to_dataset(name='annual_mean_ghi')
    masked_ghi_mean = masked_ghi_mean.assign_coords({'year':year})
    ...
    masked_ghi_mean.to_netcdf(f'{file_path}/{dataset}-{year}.nc')
